...
*  call to setdebug_traps() is necessary in order to allow any breakpoints
...
static int* stackPtr __attribute__((used))  = &remcomStack[STACKSIZE/sizeof(int) - 1] ;
...
SAVE_FP_REGS();
asm("\n"
"        lea     registers,Ta5   /* get address of registers     */\n"
  ...
  while (1 == 1)
    {
      ...
      switch (*ptr++)
        ...
        case '?':
          ...
          break;
        case 'd':
          remotedebug = !(remotedebug);        /* toggle debug flag */
          break;
...
setdebug_traps ()
{
  ...
  /* This is a trap #8 instruction.  Apparently it is someone's software
     ...
     people are being screwed by having this code the way it is?
     Is there a clean solution?  */
  exceptionHandler (40, catchException);
