...
#define MAX_SIZE 181
uint8_t image[MAX_SIZE][MAX_SIZE];

void drawMatrix(uint16_t size) {
	for (uint16_t x=0;x<size;x++)
		for (uint16_t y=0;y<size;y++)
			if (image[x][y]==0) {
				uint16_t mult = SCREEN_HEIGHT / size;
				uint16_t screenX = SCREEN_WIDTH-SCREEN_HEIGHT + x*mult;
				uint16_t screenY = y*mult;
				for (uint16_t dx=0;dx<mult;dx++) {
					uint16_t xx = screenX+dx;
					for (uint16_t dy=0;dy<mult;dy++)
						drawPixel(xx,screenY+dy);
				}
			}
}

void drawQR(const uint8_t* freetext) {
	unsigned char message[1666] = {0};  // 244 valid up to VERSION 13-Q, 1666 valid up to VERSION 40-Q
	int16_t message_length = strlen((const char*)freetext);
